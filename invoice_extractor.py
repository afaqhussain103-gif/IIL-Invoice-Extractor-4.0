"""
IIL Invoice Extractor v2.0
Handles both old (Sales Tax Invoice) and new invoice formats
Scans recursively through Year/Month folders
"""
...
class InvoiceExtractorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("IIL Invoice Extractor v2.0")
    ...
    def create_widgets(self):
        ...
        tk.Label(main_frame, text="📁 Invoice Root Folder (Scans Year/Month subfolders):", font=("Arial", 10, "bold")).grid(
        ...
        tk.Button(source_frame, text="Browse", command=self.browse_source, width=10, bg=self.green, fg="white").pack(side=tk.LEFT)
        ...
        tk.Button(dest_frame, text="Browse", command=self.browse_dest, width=10, bg=self.green, fg="white").pack(side=tk.LEFT)
    ...
    def browse_source(self):
        folder = filedialog.askdirectory(title="Select Invoice Root Folder")
        if folder:
    ...
    def find_all_pdfs(self, root_folder):
        """Recursively find all PDFs in Year/Month folder structure"""
        pdf_files = []
    ...
    def start_extraction(self):
        ...
        self.progress.start()
        self.status_label.config(text="Scanning folders recursively...", fg="blue")
    ...
    def extract_invoices(self):
        try:
            ...
            pdf_files = self.find_all_pdfs(source)
            
            if not pdf_files:
                self.show_result("No PDF files found in folder structure!", "error")
                return
            
            self.root.after(0, lambda: self.status_label.config(text=f"Found {len(pdf_files)} PDFs. Processing...", fg="blue"))
            ...
            for idx, pdf_path in enumerate(pdf_files):
                self.root.after(0, lambda i=idx, total=len(pdf_files): 
                    self.status_label.config(text=f"Processing {i+1}/{total}...", fg="blue"))
                ...
                try:
                    ...
                    for page_num in range(len(doc)):
                        ...
                        # Detect invoice page (both old and new formats)
                        is_invoice_page = (
                            ("SALES TAX INVOICE" in text) or
                            ("Invoice Details" in text and "Number" in text) or
                            ("Invoice No" in text and "Bill To" in text)
                        )
    ...
    def extract_metadata(self, text):
        """Extract metadata from both old (Sales Tax) and new invoice formats"""
        
        # INVOICE NUMBER
        # Try OLD format: "Invoice No. SN23-0101132" or "Invoice No: SN23-0101132"
        invoice_num_match = re.search(r'Invoice No[.:\s]+([A-Z0-9\-]+)', text, re.IGNORECASE)
        
        # Try NEW format: "Number IILKP23-0123456"
        ...
        invoice_num = invoice_num_match.group(1).strip() if invoice_num_match else "UNKNOWN"
        
        
        # INVOICE DATE
        # Try OLD format: "Invoice Date. 22-JUN-2023" or "22-JUN-2023"
        date_match = re.search(r'Invoice Date[.:\s]+(\d{2}-[A-Z]{3}-\d{4})', text, re.IGNORECASE)
        
        # Try alternative old format
        if not date_match:
            date_match = re.search(r'(\d{2}-[A-Z]{3}-\d{4})', text)
        
        # Try NEW format: "Date 01-JAN-2026"
        if not date_match:
            date_match = re.search(r'Date\s+(\d{2}-[A-Z]{3}-\d{4})', text)
        
        invoice_date = date_match.group(1).strip() if date_match else "NODATE"
        
        
        # CUSTOMER NAME
        # Try OLD format: "Bill To: ZAHIR KHAN & BROTHERS"
        customer_name_match = re.search(r'Bill To[:\s]+(.+?)(?=\n\n|\n[A-Z0-9\-]|\nNTN|\nGST)', text, re.DOTALL | re.IGNORECASE)
        
        if customer_name_match:
            customer_name = customer_name_match.group(1).strip()
            # Clean up multiple lines and spaces
            customer_name = ' '.join(customer_name.split())
        else:
            # Try NEW format: "Name <customer>"
        ...
        # ACCOUNT ID
        # Try OLD format: "Customer A/C 4997"
        account_match = re.search(r'Customer A/C[:\s]+(\d+)', text, re.IGNORECASE)
        
        # Try NEW format: "Account 123456"
        if not account_match:
            account_match = re.search(r'Account\s+(\d+)', text)
        
        account_id = account_match.group(1).strip() if account_match else "NO_ID"
...
📊 Summary:
────────────────────────────────
• Scanned: {total_files} PDF file(s)
